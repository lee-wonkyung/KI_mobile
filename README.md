...
css/
  ...
  base.css              리셋·모바일 셸·상태바·홈 인디케이터
  components.css        버튼·입력폼·카드·헤더·칩·자리표시 박스 등 재사용 컴포넌트
screens/
  01-intro.html         인트로
  02-guide.html         테스트 안내
  03-permission.html    카메라 권한(하단 시트)
  04-test.html          문항 O/X
  05-loading.html       성향 분석 중
...
- 색상·여백·폰트 크기는 디자인 수치를 토큰으로 반영
- 레이아웃은 Flex/Grid 기반으로 재구성 (절대좌표 미사용)
- **시스템 한글 폰트 스택** 대신 **Pretendard** 웹폰트 사용 (jsDelivr CDN)
- 반복 요소(버튼·입력폼·카드)는 `components.css`에 재사용 컴포넌트로 분리, **케밥케이스** 네이밍
- **이미지·아이콘은 Figma 자산을 PNG/SVG로 처리**. 콘텐츠 이미지(캐릭터·카드·가챠·상품 등)는 `assets/img/`의 실제 PNG, 헤더·체크·화살표 등 UI 아이콘은 인라인 SVG, 아이콘 박스는 디자인과 동일하게 이모지를 사용했습니다.

## 참고
- 키보드 영역은 CSS로 구현했습니다(실제 OS 키보드 대체).
